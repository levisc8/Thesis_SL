...
# list. Additionally, the Rumex genus is not in GBOTB phylogeny at all,
# and I'm not entirely sure where to insert it. For now, removing it

omit_spp <- c('Rumex_crispus', 'Rumex_acetosella', 'Cerastrium_spp.')
exotics <- filter(spp.list, Exotic == 1 & !Species %in% omit_spp)
...
for(x in unique(demo.data$Species)) {
  ...
  local.exotics <- filter(use_com, exotic_species == x &
                            alien == 1)
...
write.csv(modelTable, '../Eco_Letters_Manuscript/Figures/Phylo_models_output_gb_OTB.csv', 
          na = "",
          row.names = FALSE)
...
ggsave(filename = "Figure_1_gb_OTB.pdf",
       path = '../Eco_Letters_Manuscript/Figures',
       height = 8.5,
       width = 12.5,
       units = 'in',
