...
escr_reg <- function(demo_data, novelty_data, iteration) {
  ...
  reg_data <- cbind(escr, novelty_data) %>%
    filter(Species != "Kummerowia_striata")
...
png(filename = "../Eco_Letters_Manuscript/Figures/Figure_2_minus_kstra.png",
    height = 8.5,
    width = 12.5,
    units = 'in',
    res = 600)
...
pdf(file = "../Eco_Letters_Manuscript/Figures/Figure_2_minus_kstra.pdf",
    height = 8.5,
    width = 13.5)
...
write.csv(reg_fig_data, 
          file = '../Eco_Letters_Manuscript/Figures/Phylo_models_output_minus_kstra.csv', 
          na = "",
          row.names = FALSE)
